...
| Additional diff | `2 files changed, 9 insertions(+), 18 deletions(-)` for this P0 follow-up |
...
2. Packaging parses the MIME type/subtype before using `charset` and follows
   ...
   skipped, quoted values may contain semicolons, terminate with their
   accumulated value at EOF, and suffix text after a closing quote is ignored
   until the next separator. The EOF case is covered through both sync and
   async MIME paths. Token/quoted values, case-insensitive names and first
   duplicate policy are covered. Label
...
permissive quoted/invalid MIME cases including EOF-terminated quotes,
non-ASCII label whitespace, constructor

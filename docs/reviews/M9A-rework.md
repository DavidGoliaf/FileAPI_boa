...
| Additional diff | `2 files changed, 220 insertions(+), 88 deletions(-)` for this remediation |
...
2. Packaging parses the MIME type/subtype before using `charset` and follows
   the permissive parameter algorithm: malformed individual parameters are
   skipped, quoted values may contain semicolons, and suffix text after a
   closing quote is ignored until the next separator. Token/quoted values,
   case-insensitive names and first duplicate policy are covered. Label
   normalization removes only ASCII whitespace; lookup failure falls through
   to MIME and UTF-8.
...
4. Regression tests assert actual BufferSource bytes, expanding UTF-16 and
   ...
   order/precedence, exact quota boundaries, direct BOM splits at byte 1/2,
   and public-reader multibyte boundaries.
...
The M9 suite reports 23 passing tests; M9 package unit tests report 2; M4
async reports 33 and M4 sync 21.
...
- Current production comments and current tests describe fallback/decoder
  semantics without asserting a superseded error path. The reported MIME
  permissiveness and BOM-split defects were fixed in this remediation.
...
permissive quoted/invalid MIME cases, non-ASCII label whitespace, constructor
exception precedence, no iterator closing on conversion quota, exact-limit
acceptance, line-ending lower-bound conservatism, byte snapshots, decoder BOM
splits at byte 1/2 and public split-chunk output.
...
CI run [34262671553](https://github.com/DavidGoliaf/FileAPI_boa/actions/runs/34262671553)
for remediation commit `14bc8b4` completed successfully on Windows job
`102184263576`, macOS job `102184263749` and Ubuntu job `102184263844`.
The complete CI matrix, including WPT strict runs and release gates, passed.
M9-B remains outside this work order.

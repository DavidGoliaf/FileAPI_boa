...
| 3 | `cargo test --workspace --all-features` | 0 | PASS (351 tests: 42 boa_fapi unit incl. 5 sync, 13 boa_fapi guards, 51 M2 JS integration, 33 M4-A JS integration, 20 M4-B JS integration, 28 M3-B JS integration, 16 M3-A JS integration, 1 boa_fapi doc, 146 core: 58+25+8+20+16+19) |
...
| 8 | `cargo test --package boa_fapi --test m4_filereader_sync -- --nocapture` | 0 | PASS (20 tests, every result returned synchronously) |
...
| 13 | `cargo deny check` | 0 | PASS (advisories ok, bans ok, licenses ok, sources ok; no new dependencies) |
...
- `cargo deny check` records its factual local result (exit 0: no new
  dependencies were added). Green CI never rewrites this local result.
- CI: `awaiting customer verification` — the owner checks green Windows
  and Ubuntu runs for the final commit before acceptance; no run URL/ID
  is claimed here.

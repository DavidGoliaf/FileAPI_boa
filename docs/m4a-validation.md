...
| 3 | `cargo test --workspace --all-features` | 0 | PASS (299 tests: 30 boa_fapi unit, 12 boa_fapi guards, 51 M2 JS integration, 31 M4-A JS integration, 28 M3-B JS integration, 16 M3-A JS integration, 1 boa_fapi doc, 130 core: 58+25+8+20+16+19) |
...
| 7 | `cargo test --package boa_fapi --test m4_filereader_async` | 0 | PASS (31 tests, every event observed after `context.run_jobs()`) |
| 8 | `cargo doc --workspace --no-deps` (RUSTDOCFLAGS='-Dwarnings') | 0 | PASS |
| 9 | `cargo test --package boa_fapi --doc` | 0 | PASS (1 doc test) |
| 10 | `cargo llvm-cov --package boa_fapi --all-features --fail-under-lines 85` | 0 | PASS (88.26% lines, threshold 85%) |
| 11 | `cargo hack check --feature-powerset --depth 2` | 0 | PASS (all feature combinations, incl. `--no-default-features`) |
| 12 | `cargo deny check` | 0 | PASS (advisories ok, bans ok, licenses ok incl. new BSD-3-Clause, sources ok) |
...
dom.rs                           1045               176    83.16%          53                 3    94.34%         783               105    86.59%           0                 0         -
...
filereader.rs                    1215               182    85.02%          73                16    78.08%         942               125    86.73%           0                 0         -
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
TOTAL                            5307               776    85.38%         323                49    84.83%        3782               444    88.26%           0                 0         -
```
...
- Throttle uses the injected `Clock` (fake/step clocks in tests; never
  system time); the event `timeStamp` reuses the pump's clock tick so no
  extra clock read perturbs the 50 ms accounting.
